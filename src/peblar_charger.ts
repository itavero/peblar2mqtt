...
const INTERVAL_CHECK_EV_INTERFACE = 1000 * 10; // 30 seconds
...
type ChargerMonitorContext = FsmContext<{
...
export class ChargerMonitor {
  ...
  private async requestEvInterfaceData(
    ...
    if (error) {
      ...
      if (!_.isEqual(this.ev_interface_, data)) {
  ...
  private async requestMeterData(
    context: ChargerMonitorContext
  ): Promise<void> {
